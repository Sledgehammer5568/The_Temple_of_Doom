...
he can only enter at night. Jones travels to the temple after sleeping.''')
    ...
    if Main_Character.MainCharacter.inventory == ["torch"]:
        print("""
...
a trap in the entrance. He avoids the trap and enters the temple""")
    ...
    # back to the village and buy one from the store
        ...
        section_2_options_2 = input("a. Look around the temple entrance\nb. Enter the Temple\nc. Return to village\n")

        # the player will be allowed to make their own torch without having to use money out of the resources they find
        if section_2_options_2 == "a":
            print("Jones decides to look around the entrance of the temple to see what he finds.")
            input("press 'enter' to continue\n")

            # make a fight between Jones and an easy enemy
            print("Jones encounters a monster.")
            input("press 'enter' to continue\n")
            # if player defeats enemy add money to inventory

            # after the player fights the enemy they can collect wood and cloth to build a torch
            # once they have created the torch they are allowed to enter the temple and continue to Section_3
